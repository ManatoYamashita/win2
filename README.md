...
2025-10-12時点で、ログイン不要のゲスト応募機能を実装済みです。

**応募可否**

- **会員**: ログイン必須ではありません。
  - ログインしていれば会員IDで計測。
  - 未ログインの場合はゲストとして計測（次項参照）。
- **ゲスト（未ログイン）**: そのまま応募可能。

**計測方法**

- **会員**: `?id1=<memberId>` を付与（現行どおり）。
- **ゲスト**: `?id1=guest:<UUID>` を付与（ブラウザに永続保存／1年保持）。

**クリックログ（シート「クリックログ」）**

- A列: 日時(JST)
- B列: ID（`memberId` または `guest:<UUID>`）
- C列: 案件名
- 追加列は不要（既存互換性維持）。

**CSV集計（GAS - code.gs）**

- `id1` が `guest:` で始まる行は「非会員」として扱います。
  - 還元額 = 0円
  - 氏名表記 = 「非会員」
- 既存の会員との突合は `memberId` のみを対象（ゲストは対象外）。

**将来の拡張**

- 後から「ゲストが会員化したら過去の成果を付け替えたい」運用も可能です（追加要件）。その場合は、登録フォームで「招待コード」欄に `guest:<UUID>` を入れてもらう／またはメール一致で後追い突合などの設計を別途追加します。
