...
- ✅ カスタム機能開発（Google Sheets API、GAS、キャッシュバック計算、A8.net連携）
- ✅ テスト・バグ修正・納品準備
...
- 成果CSV_RAWから報酬額を取得
- キャッシュバック率20%を適用
- 会員名、案件名、承認状況、キャッシュバック金額を「成果データ」シートへ自動転記

**計算式**
```
キャッシュバック金額 = 原始報酬額 × 0.2
```
...
**承認状況管理**
- `pending`: 未承認（申込直後）
- `approved`: 承認済み（キャッシュバック確定）
- `cancelled`: キャンセル（キャッシュバック対象外）
...
9. （将来的に）自動キャッシュバック計算・成果データシート転記
