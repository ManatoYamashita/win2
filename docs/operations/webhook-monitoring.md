...
GAS処理後のキャッシュバックデータを確認（毎日3:10更新）：
...
**確認ポイント:**
- キャッシュバック金額（D列）が報酬額 × 20%（切り捨て）になっているか
- 非会員（A列が"非会員"）のキャッシュバック金額が0円になっているか
...
### Q4: 非会員のキャッシュバックが0円になっている

**A:** これは正常な動作です：

- `guest:`で始まる追跡IDは非会員
- キャッシュバックは会員のみ対象（GAS処理で0円に設定）
- 成果自体は記録されます
