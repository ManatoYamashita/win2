...
**✅ 2025/11/15更新: onEdit()トリガーによる即時実行**

CSVを貼り付けた瞬間、自動的にキャッシュバック計算が実行されます。

**動作仕様:**
- 「成果CSV_RAW」シートにCSVを貼り付けると、`onEdit()` トリガーが自動発火
- 複数行・複数列の編集（CSV貼り付け）を検知して即座に処理実行
- 単一セルの編集は無視（誤動作防止）
- 処理時間: 通常30秒〜1分（データ量に応じて変動）

**手動実行（必要な場合のみ）:**
1. Google Sheetsメニュー: 「成果処理」→「CSV取込→集計」をクリック
2. 実行権限の確認ダイアログが表示されたら「許可」をクリック
3. 処理完了まで待機

**⚠️ 注意:**
- 旧仕様の「毎日3:10自動実行」は廃止されました
- CSV貼り付け時の即時処理が標準動作です
...
- **CSV貼り付け時**: GAS onEdit()トリガーで即時処理実行（「成果CSV_RAW」→「成果データ」変換）

**✅ 2025/11/15更新:**
- 旧仕様の「毎日3:10 JST GAS自動実行」は廃止
- A8.net CSV貼り付け時に即座に処理されるため、定期実行不要
...
**作成日**: 2025-11-15
**最終更新**: 2025-11-15 v1.1.0（onEdit()トリガー対応）
**ステータス**: 完成
**更新履歴**:
- v1.0.0 (2025-11-15): 初版作成（AFB自動ポーリング + A8.net手動CSV）
- v1.1.0 (2025-11-15): GAS onEdit()トリガー対応、時間ベーストリガー削除
